                     ...
                     if ($query->rowCount() != 0) {
                            ...
                            foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $papara) {
                               $sim = similar_text(strtolower(turkceDegistir($isim)), strtolower(turkceDegistir($papara['hesap_adi'])), $perc);
                                ...
                                if ($perc > 85) {
                                    $json = json_decode(file_get_contents('http://software2.betkanyon100.com/bonus/ajax/papara_tanimla_oto.php?hesap_adi='.urlencode($isim).'&islem_id='.$papara['id'].'&tutar='.$tutar.'&status=accept'),true);
                                    if ($json['status'] == 'success') {
                                        
                                        imap_setflag_full($inbox, $mail, "\\Seen \\Flagged");
                                        
                                        
                                        $insert = $db->prepare('INSERT INTO islenen_mailler (papara_id,mail_id,tarih) values (?,?,?)');
                                        $insert->execute(array($papara_id,$headerInfo->udate,date('Y-m-d H:i:s')));
                                    }
                                } else {
